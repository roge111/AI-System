...
Линейная регрессия — один из самых простых и распространенных методов обучения. Идея в том, что у нас есть некоторые данные. Метод проводит анализ данных. И на их основе предполагает свое мнение. Например, у нас есть пара X и Y, где X — площадь квартиры, Y — цена.
Таких пар у нас несколько. И на их основе метод пытается построить некую прямую, приближенную к исходным результатам. Если говорить просто — найти закономерность в обучающей выборке.
Выглядет это так:
...
Мы хотим, чтобы разница между реальным значением Y и предсказанным была как можно меньше. Такая разница называется `ошибкой`. Для этого используется метод наименьших квадратов с формулой:
...
## Если признаков для предсказания больше ?
---
Тут используется множественная линейная регрессия.


Создается матрица признаков. Матрица может в себе содержать сразу несколько случаев, которые мы хотим предсказать. 
И у нас есть целевые переменные, которые тоже выглядят как столбец. А затем используетм формулу:
<img width="139" height="31" alt="image" src="https://github.com/user-attachments/assets/b0045ade-bcd3-4f2b-bac8-6fa686d70bf1" />

По это формуле мы находим свободные члены для каждого случая (это на первом месте списка) и коэффициенты признаков для каждого случая. 
...
Метод k-ближайших соседей (k-NN) — это простейший метод и интуитивно понятный. Тут не так страшно. Логика такого метода очень проста. Мы смотрим на k ближайших элементов (соседей) и на их основе даем результат. Метод применим как для задач классификации, так и для регрессии.
